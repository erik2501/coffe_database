...
#README
#Kjør programmet og main funksjon vil opprette database, samt gi deg tilgang til de ulike brukerhistoriene
#Det opprettes også testdata for å ha resultater å vise til ved løsning av brukerhistorier
#Hvis main funksjon kjøres på nytt vil eventuelle smakinger og nye brukere slettes for å unngå at testdata som legges inn dobbles
#Dette betyr at hvis man øsnker å sjekke innlogging funksjonen må dette gjøres med eksisterende testbrukere
#Validering har ikke blitt tatt hensyn til så pass på å skrive riktig i konsoll
#skulle det oppstå problemer har vi lagt til støttefuksjoner som kan slette all data i databsen og opprette testdata
#README
...
def main():
    ...
    while login is True:
        ...
        if bruker == 'N':
            epost = input('E-post: ')
            passord = input('Passord: ')
            sql = "SELECT * FROM bruker WHERE bruker.epost = ? AND bruker.passord = ?"
            cursor.execute(sql, (epost, passord))
            brukers = cursor.fetchall()
            if len(brukers) == 1:
                user_login = brukers[0][0]
                login = False
            else:
                print('Obs brukernavn eller passord er feil!')
